...
def makeMaskedCNN(inshape, cnn_arch, dense_arch, activation, seed, initializer, masktype, trainW, trainM, p1, abg):
    inputshape = inshape
    arch = dense_arch
...
def makeMaskedMLP(fullarch, activation, seed, initializer, masktype, trainW, trainM, p1, abg):
    ...
    if len(arch) == 0:
        LN = MaskedDense(outshape, 'softmax', seed, initializer, masktype, trainW, trainM, p1, abg)(input_img)

    # if there are hidden layers then
    else:
        # add the first hidden layer and connect it to the input
        Li = MaskedDense(arch[0], activation, seed, initializer, masktype, trainW, trainM, p1, abg)(input_img)

        # add the rest of the hidden layers (if any) and connect
        # them to the previous ones
        for i in range(1, len(arch)):
            Li = MaskedDense(arch[i], activation, seed, initializer, masktype, trainW, trainM, p1, abg)(Li)

        # here is the last layer, connected to the one before
        # (either the ones from the loop or the one before)
        LN = MaskedDense(outshape, 'softmax', seed, initializer, masktype, trainW, trainM, p1, abg)(Li)
